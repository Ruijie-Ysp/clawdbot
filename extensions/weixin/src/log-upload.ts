...
type CliCommand = {
  ...
  action<TArgs extends unknown[]>(fn: (...args: TArgs) => void | Promise<void>): CliCommand;
};
...
function getConfiguredUploadUrl(config: OpenClawConfig): string | undefined {
  const section = config.channels?.["weixin"] as { logUploadUrl?: string } | undefined;
  return section?.logUploadUrl;
}

/** Register the `openclaw weixin logs-upload` CLI subcommand. */
export function registerWeixinCli(params: { program: CliCommand; config: OpenClawConfig }): void {
  const { program, config } = params;

  const root = program.command("weixin").description("Weixin channel utilities");
