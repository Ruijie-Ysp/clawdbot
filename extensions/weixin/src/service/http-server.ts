...
export class WeixinDemoHttpServer {
  ...
  async start(): Promise<void> {
    if (!this.serviceConfig.enabled) {
      this.logger.info("[weixin] demo service disabled");
      return;
    }
    if (this.server) {
      return;
    ...
    this.logger.info(
      `[weixin] demo service listening on http://${this.serviceConfig.bind}:${this.serviceConfig.port}`,
    );
  }
  ...
  private async handleRequest(req: IncomingMessage, res: ServerResponse): Promise<void> {
    try {
      const url = new URL(
        req.url || "/",
        `http://${this.serviceConfig.bind}:${this.serviceConfig.port}`,
      );
      ...
      const qrStatusMatch =
        req.method === "GET" ? url.pathname.match(/^\/api\/qr\/([^/]+)\/status$/) : null;
      if (qrStatusMatch) {
      ...
      const reloginMatch =
        req.method === "POST" ? url.pathname.match(/^\/api\/accounts\/([^/]+)\/relogin$/) : null;
      if (reloginMatch) {
      ...
      this.logger.error(`[weixin] demo service error: ${message}`);
