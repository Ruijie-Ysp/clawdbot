...
export function resolveWeixinDemoServiceConfig(config: OpenClawConfig): WeixinDemoServiceConfig {
  const channels = (config as Record<string, unknown>).channels as
    ...
    | undefined;
  const section = channels?.["weixin"] as Record<string, unknown> | undefined;
  const demoService = section?.demoService as Record<string, unknown> | undefined;

  return {
    enabled:
      typeof demoService?.enabled === "boolean" ? demoService.enabled : DEFAULT_CONFIG.enabled,
    bind:
      typeof demoService?.bind === "string" && demoService.bind.trim()
