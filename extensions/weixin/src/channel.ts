...
async function sendWeixinOutbound(params: {
  ...
  if (!account.configured) {
    ...
    throw new Error("weixin not configured: please run `openclaw channels login --channel weixin`");
  }
...
export const weixinPlugin: ChannelPlugin<ResolvedWeixinAccount> = {
  ...
  outbound: {
    ...
    sendMedia: async (ctx) => {
      ...
      if (!account.configured) {
        ...
        throw new Error(
          "weixin not configured: please run `openclaw channels login --channel weixin`",
        );
      }
  ...
  auth: {
    login: async ({ cfg, accountId, verbose, runtime }) => {
      ...
      log(`\nScan the QR code below in Weixin to finish login:\n`);
      try {
        ...
        log(`QR URL: ${startResult.qrcodeUrl}`);
      }
  ...
  gateway: {
    startAccount: async (ctx) => {
      ...
      if (!account.configured) {
        ...
        ctx.log?.error?.(
          `[${account.accountId}] weixin not logged in — run: openclaw channels login --channel weixin`,
        );
