...
export async function loadExecApprovals(
  ...
  try {
    ...
    if (!rpc) {
      state.lastError = t("nodes.execApprovals.errors.selectNodeLoad");
      return;
    }
...
export async function saveExecApprovals(
  ...
  try {
    ...
    if (!baseHash) {
      state.lastError = t("nodes.execApprovals.errors.hashMissing");
      return;
    }
    ...
    if (!rpc) {
      state.lastError = t("nodes.execApprovals.errors.selectNodeSave");
      return;
    }
