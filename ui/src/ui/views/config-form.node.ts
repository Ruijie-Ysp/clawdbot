...
/**
 * Translate a field name using configForm.fields translations
 * Falls back to humanize if no translation exists
 */
export function translateFieldLabel(fieldName: string): string {
  // Try direct translation
  const key = `configForm.fields.${fieldName}`;
  if (hasTranslation(key)) {
    return t(key);
  }

  // Try camelCase conversion (e.g., "block_streaming_break" -> "blockStreamingBreak")
  const camelCase = fieldName.replace(/_([a-z])/g, (_, c) => c.toUpperCase());
  const camelKey = `configForm.fields.${camelCase}`;
  if (hasTranslation(camelKey)) {
    return t(camelKey);
  }
...
export function renderNode(params: {
  ...
  return html`
    <div class="cfg-field cfg-field--error">
      <div class="cfg-field__label">${label}</div>
      <div class="cfg-field__error">${tp("configForm.unsupportedType", { type: String(type) })}</div>
    </div>
  `;
}
  ...
  const placeholder =
    ...
    (isSensitive
      ...
      : schema.default !== undefined
        ? tp("configForm.defaultPlaceholder", { value: String(schema.default) })
        : "");
    ...
    <div class="cfg-array">
      <div class="cfg-array__header">
        ...
        <span class="cfg-array__count">
          ${
            arr.length === 1
              ? tp("configForm.itemsCountSingle", { count: 1 })
              : tp("configForm.itemsCountPlural", { count: arr.length })
          }
        </span>
        <button
          type="button"
                    ...
                    anySchema
                      ? html`
                        <textarea
                          ...
                          placeholder="${t("configForm.jsonPlaceholder")}"
                          rows="2"
                          .value=${fallback}
                          ?disabled=${disabled}
                          @change=${(e: Event) => {
