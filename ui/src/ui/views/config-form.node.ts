  ...
  return html`
    <div class="cfg-field cfg-field--error">
      ...
      <div class="cfg-field__error">
        ${tp("configForm.unsupportedType", { type: String(type) })}
      </div>
    </div>
  `;
}
...
function renderArray(params: {
  ...
  const countLabel =
    arr.length === 1
      ? tp("configForm.itemsCountSingle", { count: String(arr.length) })
      : tp("configForm.itemsCountPlural", { count: String(arr.length) });

  return html`
    <div class="cfg-array">
      <div class="cfg-array__header">
        ${showLabel ? html`<span class="cfg-array__label">${label}</span>` : nothing}
        <span class="cfg-array__count">${countLabel}</span>
        <button
          type="button"
