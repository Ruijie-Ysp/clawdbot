...
export function renderLogs(props: LogsProps) {
  ...
  const exportLabelKey = needle || levelFiltered ? "filtered" : "visible";
  const exportLabel = t(`logs.exportScope.${exportLabelKey}`);
    ...
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        ...
        <div class="row" style="gap: 8px;">
          ...
          >
            ${tp("logs.exportLabel", { scope: exportLabel })}
          </button>
        </div>
      </div>
      ...
      ${props.file
        ? html`<div class="muted" style="margin-top: 10px;">${tp("logs.fileLabel", { file: props.file })}</div>`
        : nothing}
      ${props.truncated
