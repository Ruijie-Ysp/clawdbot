...
export function renderDebug(props: DebugProps) {
  ...
  const securityLabel =
    critical > 0
      ? tp("debug.securityAudit.critical", { count: critical })
      : warn > 0
        ? tp("debug.securityAudit.warnings", { count: warn })
        : t("debug.securityAudit.none");
  const infoSuffix = info > 0 ? tp("debug.securityAudit.info", { count: info }) : "";
  const securitySummaryText = tp("debug.securityAudit.summary", {
    ...
    <section class="grid grid-cols-2">
      <div class="card">
        ...
        <div class="stack" style="margin-top: 12px;">
          <div>
            <div class="muted">${t("debug.statusLabel")}</div>
            ${
              securitySummary
          ...
          <div>
            <div class="muted">${t("debug.healthLabel")}</div>
            <pre class="code-block">${JSON.stringify(props.health ?? {}, null, 2)}</pre>
          </div>
          <div>
            <div class="muted">${t("debug.heartbeatLabel")}</div>
      ...
      <div class="card">
        ...
        <div class="row" style="margin-top: 12px;">
          <button class="btn primary" @click=${props.onCall}>${t("debug.callButton")}</button>
        </div>
        ${
          props.callError
              ...
              ${props.eventLog.map(
                (evt) => html`
                  <div class="card" style="padding: 12px;">
                    ...
                    <pre class="code-block" style="margin-top: 8px;">${formatEventPayload(evt.payload)}</pre>
                  </div>
                `,
              )}
            </div>
