  ...
  return html`
    ...
    <section class="card" style="margin-top: 18px;">
      <div class="row" style="justify-content: space-between;">
        ...
        <div class="muted">${formatAgo(props.lastSuccessAt)}</div>
      </div>
      ${
        props.lastError
...
function resolveChannelLabel(snapshot: ChannelsStatusSnapshot | null, key: string): string {
  ...
  const translationKey = `channels.${key}`;
  return (
    snapshot?.channelLabels?.[key] ??
    (t(translationKey) === translationKey ? key : t(translationKey))
  );
}
    ...
    <div class="account-card">
      ...
      <div class="status-list account-card-status">
        ...
        <div>
          ...
          <span>${formatAgo(account.lastInboundAt)}</span>
        </div>
        ${
          account.lastError
            ? html`
